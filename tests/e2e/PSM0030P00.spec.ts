...
test.describe('PSM0030P00 기술등급이력조회 E2E', () => {
	...
	test('기술등급이력조회', async ({ page }) => {
		...
		await test.step('등급이력조회 팝업 대기', async () => {
			try {
				// 여러 가지 팝업 선택자 시도
				await Promise.race([
				...
				console.log('등급이력조회 팝업이 나타났습니다')
			} catch (e) {
				console.log('등급이력조회 팝업이 나타나지 않았습니다. 직접 계산 버튼을 찾아보겠습니다.')
			}
		})
	});
