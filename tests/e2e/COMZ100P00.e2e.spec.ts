...
test.describe('COMZ100P00 사용자명 검색 팝업 E2E', () => {
  ...
  test('데이터 더블클릭 시 부모창으로 파라미터 전달 테스트', async ({ page, context }) => {
    // 부모창 시뮬레이션을 위한 메인 페이지
    const mainPage = page;
    
    await test.step('부모창에서 postMessage 리스너 설정', async () => {
      // 부모창에서 자식창으로부터 메시지를 받을 리스너 설정
    ...
    await test.step('자식창(팝업) 생성', async () => {
      // 팝업 창 생성
      ...
      await test.step('팝업에서 초기 상태 확인', async () => {
        // 팝업이 정상적으로 로드되었는지 확인
        await expect(popupPage.locator('.popup-header')).toBeVisible();
        await expect(popupPage.getByText('사용자명 검색')).toBeVisible();
      });

      await test.step('팝업에서 사용자명 입력', async () => {
        await popupPage.getByRole('textbox', { name: '사용자명 입력' }).fill('차준형');
        await expect(popupPage.getByRole('textbox', { name: '사용자명 입력' })).toHaveValue('차준형');
      });

      await test.step('팝업에서 조회 버튼 클릭', async () => {
      ...
      await test.step('검색 결과 행 더블클릭', async () => {
        const firstRow = popupPage.locator('.grid-table tbody tr').first();
        ...
        // 더블클릭 실행 전 postMessage 리스너 준비
        await popupPage.evaluate(() => {
          // postMessage 전송을 감지하기 위한 플래그 설정
          (window as any).messageSent = false;
          const originalPostMessage = window.opener?.postMessage;
          if (window.opener?.postMessage) {
            window.opener.postMessage = function(message, targetOrigin, transfer) {
              (window as any).messageSent = true;
              (window as any).sentMessage = message;
              console.log('postMessage 전송됨:', message);
        ...
        // postMessage 전송 확인
        const messageSent = await popupPage.evaluate(() => {
          return (window as any).messageSent;
        });
        
        console.log('postMessage 전송 여부:', messageSent);
        
        // 팝업이 닫히는지 확인 (오류 무시)
        try {
          await expect(popupPage.isClosed()).toBeTruthy();
        } catch (error) {
          console.log('팝업 닫힘 확인 중 오류 (정상):', error.message);
        }
      });

      await test.step('부모창에서 전달된 파라미터 확인', async () => {
        // 부모창으로 돌아가서 받은 메시지 확인
