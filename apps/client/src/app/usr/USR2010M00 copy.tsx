...
const USR2010M00: React.FC = () => {
	...
	useEffect(() => {
		if (userData && userData.length === 0) {
			...
			// Flex 소스와 동일하게 빈 사용자 ID로 업무권한 조회
			usrApiService.getWorkAuthList("").then((list) => {
			...
			<div className='gridbox-div mb-4'>
				<DataGrid
					...
					columnDefs={[
						...
						{ headerName: "본부명", field: "hqDivNm" },
						{ headerName: "부서명", field: "deptDivNm" },
						{ headerName: "직급명", field: "dutyNm" },
						{ headerName: "직책구분", field: "dutyDivCdNm" },
						{ headerName: "사용자권한", field: "authCdNm" },
						{ headerName: "사용자역할ID", field: "usrRoleId" },
						{ headerName: "사용자역할", field: "usrRoleNm" },
						{ headerName: "승인결재자", field: "apvApofNm" },
						{
							headerName: "사업",
							field: "bsnUseYn",
					...
					onRowSelected={(selectedRows) => {
						if (selectedRows.length > 0) {
							handleUserSelect(selectedRows[0]);
						}
					}}
				/>
			</div>
