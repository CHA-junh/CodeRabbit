...
export default function EmployeeMainPage() {
  ...
  const [selectedEmployee, setSelectedEmployee] = useState<EmployeeListData | null>(null); // 선택된 사원
  ...
  const columnDefs = useMemo<ColDef[]>(() => [
    ...
    { 
      ...
      // 퇴사자/휴직자 스타일 적용
      cellClassRules: {
        'retired-row': (params) => params.data?.WKG_ST_DIV === '퇴사' || params.data?.WKG_ST_DIV_CD === '3',
        'leave-row': (params) => params.data?.WKG_ST_DIV === '휴직' || params.data?.WKG_ST_DIV_CD === '2',
      }
    },
    ...
    { headerName: '최종투입일', field: 'LAST_IN_STRT_DT', width: 100, hide: true },
    { headerName: '최종철수일', field: 'LAST_IN_END_DT', width: 100, hide: true },
    { headerName: '최종프로젝트', field: 'LAST_PRJT', width: 150, hide: true },
  ...
  const handleHqDivChange = async (hqDiv: string) => {
    try {
      ...
      // 서버 데이터에서 ALL코드를 제거하고 클라이언트에서 맨 위에 추가
      const filteredDeptData = deptData?.filter((item: any) => item.DATA !== 'ALL') || [];
      const finalDeptData = [{ DATA: 'ALL', LABEL: '전체' }, ...filteredDeptData];
      ...
      if (response.ok) {
        ...
        if (result.success) {
          ...
          if (result.data && result.data.length > 0) {
            showToast(`${result.data.length}명의 사원이 조회되었습니다.`, 'info');
          } else {
  ...
  const handleProjectInquiry = () => {
    if (!selectedEmployee) {
      showToast('사원(외주) 리스트에서 대상자를 선택 클릭해 주십시요.', 'warning');
      return;
    }
  ...
  return (
    <div className="mdi flex flex-col h-[calc(100vh-200px)] overflow-hidden min-w-[1400px]">
            ...
            <tr>
              ...
              <td className="search-td w-[150px]">
                <input 
                  ...
                  onChange={(e) => setSearchConditions(prev => ({ ...prev, empNm: e.target.value }))}
                  onKeyPress={(e) => e.key === 'Enter' && handleSearch()}
                />
              </td>
              <th className="search-th w-[60px]">{searchConditions.ownOutsDiv === '2' ? '외주업체' : '본부'}</th>
              <td className="search-td w-[150px]">
                <select 
              ...
              <td className="search-td w-[150px]">
                <select 
                  ...
                  onChange={(e) => setSearchConditions(prev => ({ ...prev, deptDiv: e.target.value }))}
              ...
              <td className="search-td w-[130px]">
                <select 
                  ...
                  onChange={(e) => setSearchConditions(prev => ({ ...prev, duty: e.target.value }))}
                >
      ...
      {/* AG Grid */}
      <div className="ag-theme-alpine flex-1 min-h-0" style={{ height: '400px' }}>
        <AgGridReact
          ...
          getRowClass={(params) => {
            ...
            if (workStatus === '퇴사' || workStatusCd === '3') classes.push('retired-row');
            else if (workStatus === '휴직' || workStatusCd === '2') classes.push('leave-row');
            return classes;
          }}
      ...
      <div className="flex flex-col flex-1 min-h-0">
        ...
        <div className="tab-panel flex-1 min-h-0 overflow-auto">
          {activeTab === 0 && <PSM1020M00 ref={psm1020M00Ref} selectedEmployee={selectedEmployee} fieldEnableState={getFieldEnableState(selectedEmployee)} onSearchSuccess={handleSearch} />}
          {activeTab === 1 && <PSM1030M00 ref={psm1030M00Ref} selectedEmployee={selectedEmployee} />}
          {activeTab === 2 && <PSM1040M00 selectedEmployee={selectedEmployee} />}
          {activeTab === 3 && (
            <PSM0050M00 
              isTabMode={true}
