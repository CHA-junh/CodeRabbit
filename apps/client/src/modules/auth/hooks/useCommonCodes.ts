...
export function useCommonCodes(): UseCommonCodesReturn {
	...
	const fetchHqDivCodes = async () => {
		try {
			...
			setError(err instanceof Error ? err.message : '본부구분코드 조회 중 오류가 발생했습니다.')
			setHqDivCodes([])
		} finally {
			setLoading(false)
		}
	...
	const fetchDeptDivCodes = async () => {
		try {
			...
			const response = await fetch(`${API_BASE_URL}/api/common/dept-div-codes`, {
				method: 'GET',
				headers: {
					'Content-Type': 'application/json',
				},
			...
			setError(err instanceof Error ? err.message : '부서구분코드 조회 중 오류가 발생했습니다.')
			setDeptDivCodes([])
		} finally {
			setLoading(false)
		}
